...
<div class="allies-wrapper">
    ...
    <div class="table-container">
        <div class="table-responsive" style="padding: 2rem;">
            <form action="{{ route('admin.commercial-allies.update', $commercialAlly->id) }}" method="POST" enctype="multipart/form-data" id="editForm">
                @csrf
                @method('PUT')
                ...
                <div class="form-grid">
                    ...
                    <div class="full-width">
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('name', $commercialAlly->name) }}" 
                                   required>
                            @error('name')
                    ...
                    <div>
                        <div class="form-group">
                            <label for="rating">
                                Rating <span class="tooltip-icon" data-tooltip="Valoración de 0.0 a 5.0">?</span>
                            </label>
                            <div style="position: relative;">
                                <input type="number" 
                                       ...
                                       placeholder="Ej: 4.5" 
                                       value="{{ old('rating', $commercialAlly->rating) }}">
                    ...
                    <div>
                        <div class="form-group">
                            <label for="logo">
                                ...
                                <span class="tooltip-icon" data-tooltip="Dejar en blanco para mantener el actual">?</span>
                            </label>
                            <input type="file" 
                                   name="logo" 
                                   id="logo" 
                                   class="form-control-file @error('logo') error @enderror"
                                   accept="image/*">
                            @error('logo')
                                <span class="error-message"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                            @enderror
                                ...
                                @if($commercialAlly->logo_url)
                                    <div class="current-logo-preview" id="current-logo-preview">
                                        ...
                                        <img id="current-logo-img" 
                                             src="{{ $commercialAlly->logo_url }}" 
                                             alt="Logo actual de {{ $commercialAlly->name }}"
                                             class="logo-preview">
                                    </div>
                                @endif
                    ...
                    <div class="full-width">
                        <div class="form-group">
                            <label for="description">Descripción <span class="required-badge">Opcional</span></label>
                            <textarea name="description" 
                                      id="description" 
                                      class="form-control @error('description') error @enderror"
                                      rows="4" 
                                      placeholder="Una breve descripción sobre el aliado comercial, sus servicios o productos...">{{ old('description', $commercialAlly->description) }}</textarea>
                            @error('description')
                                <span class="error-message"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="full-width">
                        <div class="form-group">
                            <label for="website_url">URL del Sitio Web <span class="required-badge">Opcional</span></label>
                            <input type="url" 
                                   name="website_url" 
                                   id="website_url" 
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (logoInput) {
            logoInput.addEventListener('change', function(event) {
                ...
                if (file) {
                    ...
                    // Ocultar la previsualización del logo actual
                    if (currentPreview) {
        ...
        // Validación en tiempo real del rating
